describe "expression", -> describe "functionPluralities", -> describe "reduce", ->
    ...
    run = (config) -> describe config.description, ->
        ...
        if config.throws
            it "throws the expected exception", -> (expect -> reduce inputCopy, 37, 86).toThrow config.throws
        else
            it "returns the expected result", -> (expect reduce inputCopy, 37, 86).toEqual config.output
            
        beforeEach ->
            try
                reduce inputCopy, 37, 86
