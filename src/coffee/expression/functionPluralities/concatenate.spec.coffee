describe "expression", -> describe "functionPluralities", -> describe "concatenate", ->
    ...
    run = (config) -> describe config.description, ->
        ...
        if config.throws
            it "throws the expected exception", -> (expect -> concatenate inputCopy, 37, 86).toThrow config.throws
        else
            it "returns the expected result", -> (expect concatenate inputCopy, 37, 86).toEqual config.output
            
        beforeEach ->
            try
                concatenate inputCopy, 37, 86
